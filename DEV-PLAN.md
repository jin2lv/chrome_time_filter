...
- [ ] 新 Profile 完成授权、引导、设置、失效保护、诊断和性能验收（= VERIFY-CHECKLIST §13 + §2.1-2.2，依赖商店包——ZIP 已产出见 P2-23；**本项为闸门最后一项**）
- [ ] 通过后方可开始 P2-17 至 P2-20 的通用能力建设；完成后再开始 P2-21 其他金融平台适配（历史注记：08-18 曾因 P2-16 被风控阻塞「有限开闸」做过通用增强与两个内置适配包，见该批次记录）
...
| 6 | 全新 Profile 首次访问雪球 → Popup 显示「未授权」→ 授权后生效 | [ ]（§2.1/2.2/§13 仍欠，依赖商店包 + 全新 Profile） |
| 7 | 设置页「已授权站点管理」可列出/移除授权 | [ ]（◐ 2026-09-08 截图证据：设置页「已授权站点」列表与「移除」按钮正常渲染（store-assets/5-options.png）；移除联动链路受 §2.4 dist-test 机制限制，待商店包复验） |
...
| 12 | 全新 Chrome Profile：加载打包 ZIP → 完整走引导→过滤→关闭→评论→失效模态流程 | [ ]（§13，闸门最后一项） |
