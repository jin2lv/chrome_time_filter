...
**窗口已处于文档底部时重复触发补拉会误判「已到信息流末页」**：`window.scrollTo({top: scrollHeight})` 在已到底时为空操作 → 站点不再懒加载新行 → 连续 `end_stall_count=2` 屏即进 exhausted（实测第 2 屏即判「末页」，而信息流实际仍有内容）；且 exhausted 后按钮禁用，用户需切换类目/改设置才能重挂载重试。已在 `feed-backfill.ts` 头部「已知限制」记录，留待 P2-21 统一获取策略时处理（候选修法：滚动前先回退一屏，或改为滚动到可视区底部并显式派发 scroll 事件）。
...
- 遗留（下一真机批次）：①**dist-store 形态**验证 P2-20 授权弹窗/「授权全部金融站点」/撤销 4 项 + §13 商店形态 E2E + §11 远程适配包真机观察（需切换构建并走授权链路）；②§5.1 其余类目区间版按需补测（本批仅视频类）；③详情页评论回归；④P2-17 取消态与 10 屏上限真机复测；⑤§10.1–10.3 其他平台真机验收（需各站登录态）。
