...
## 2026-09-16/17 批次十二（dist-store 形态真机批：P2-20 授权链路 + §11 远程适配包 + 过滤基线）

> 前置：用户手动停用 `dist-test`、加载 `dist-store`（无 key、无预授予权限，商店形态；ID `kmgmkajopgjhpgojbgpdfkjighglemdh`——**由路径 UTF-16LE 的 SHA-256 前 128 位按 nibble 映射 a–p 反推得出**，用已知 `dist-test` ID 校准，避免从截图误读 ID，见「取证方法」），并在弹窗中点「允许」完成首次授权。全程零风控。

### 1. P2-20 多站授权与每站设置（界面级 + 行为级均通过）

- **设置页矩阵**：列出全部 4 个内置平台及授权状态（雪球 / 同花顺 / 集思录（双域）/ 东方财富资讯）；未授权行 `[授权]`、已授权行 `[移除]`；每行附数据驱动的适配包版本 + 最后验证日期 + 能力摘要（雪球 `v0.4.1 · 验证于 2026-09-16 · 六项能力`，其余三站如实标「未真机验证」）；雪球行另有「页面类型能力 (3 类)」展开项、「已记忆时间设置：截止 · 隐藏」+「重置此站设置」；底部另有「其他已授权来源（localhost）」与撤销说明文案。
- **「授权全部金融站点」（用户发起的批量授权）**：点击后**仅弹一次** Chrome 权限请求，展开「显示详细信息」为一个多站点列表（逐条读到 `finance.eastmoney.com`、`t.10jqka.com.cn`、`www.jisilu.cn`、`xueqiu.com` 及被截断的 `jisilu.cn`）；用户点「允许」后四行**全部转为已授权**——**§2.3 集思录双域「无授权死结」随之真机闭环**（一次 request 同时授予两 origin）。
- **per-site 撤销**：雪球行点「移除」→ 即时变回 `[未授权] [授权]`；**未刷新**切回个股页，虚拟分页接管与过滤全部撤除、页面回原生形态（与 09-09 §2.4 的 `permissions.remove` 链路一致，本次为 P2-20 新 UI 路径复核）。
- **过滤基线实证（同一页面内改设置做对照）**：个股页在截止=**今日0点**时可见「昨天 20:49 / 19:37 / 17:06」；改为**昨日0点**后**未刷新即时变化**——「昨天」帖全部消失、只剩 09-14 及更早——授权生效 + 过滤正确。

### 2. §11 远程适配包热更新（真机闭环）

- Service Worker 控制台（DevTools for SW，不涉及雪球页面，规避其反调试）读取 `chrome.storage.local`：
  **`adapters.remote = {"package":{…4 平台…},"updatedAt":1789570549793,"version":"1.0.0"}`** ⇒ 安装时拉取 → sha256 完整性校验 → schema 校验 → 写入存储 → 生效，**全链路真机成立**。
- `curl -I https://cdn.jsdelivr.net/gh/jin2lv/chrome_time_filter@main/adapters.json` → **HTTP/2 200**，排除「网络不可达导致静默降级」的解释。

### 3. 新发现（重要，运维纪律）：**远程包会遮挡更新的内置包**
...
1. **扩展 ID 不要从截图读**：未打包扩展 ID = `SHA-256(绝对路径的 UTF-16LE 编码)` 前 128 位，每 nibble 映射 `a`–`p`。用已知 `dist-test` ID 校准编码形式后即可精确推出 `dist-store` 的 ID（本次据此纠正了一次误读——误读的 ID 会导致 `chrome-extension://…` 导航被 Chrome 拦截为 `ERR_BLOCKED_BY_CLIENT`）。
2. **DevTools for Service Worker 的可用姿势**：`chrome://extensions` 卡片上的「Service Worker」链接打开专用 DevTools 窗口；控制台输入行在面板底部状态栏上方，**先点击输入行再输入**（`app_ref` 前台键入在 Windows 要求目标为前台窗口）。读长 JSON 不如改用**无障碍树读文本**（与探针浮层同一思路）——本批正是用 a11y 才拿到 `adapters.remote` 的完整内容。
3. **雪球页面无法点类目标签**：本批不再依赖类目切换（`#tm-tab-*` hash 通道属临时探针，已移除）；改以**直接导航到个股页 URL** 完成过滤对照，避开坐标点击与反调试两个坑。

### 5. 遗留

- **推送 `adapters.json` + `.sha256` 到 GitHub**（外向操作，待用户确认）→ 之后可观察一次真实「远程版本递增」热更新。
- §13 全新 Profile 端到端本轮未重跑（09-09 商店包已闭环；本批用的是既有 Profile）。
- 上一批遗留不变：补拉修复的真机复验、§5.1 其余类目区间版、详情页评论、取消态与 10 屏上限。

---

## 2026-09-16 批次十一（P2-17 切片 2 真机回归 + 「加载更多」取证 + O1 区间复核 + P2-18 预设交易日验收）

> 前置：用户移除全部时光机副本后手动加载 `dist-test`（ID `hmeklmafbbcpmbjikjaffgnnejgeglfe`），登录态雪球首页。全程零风控（类目切换间隔 ≥5s、无深回溯）。

### 1. 「加载更多」按钮取证与配置决策（用户点名任务）

- **候选元素取证（登录态首页，7x24 与关注/全部两上下文）**：`a.home__timeline__more`（文本「加载更多」）；同级末页标记 `span.home__timeline__no-more`（「没有更多内容了」）；另有 `div.timeline__empty`（空类目占位）。
- **关键事实：两上下文补拉全程这两个元素均 `display:none`、0×0**。雪球首页 7x24 与关注流均**由滚动触发无限加载**（实测滚动后行数 10→30、15→30），**不存在可见的按钮式获取**。
- **处置**：①适配包按**防御性声明** `load_more_selector: "a.home__timeline__more"`（引擎 `canLoadMore()` 具备存在/可见/disabled 三重门控，不可见时绝不点击，故无副作用）；②`xueqiu.json` 升 **v0.4.1**、`last_verified=2026-09-16`，notes 补记本次复核结论；③`npm run adapters:build` 重生成 `adapters.json` + `.sha256`（同版本号 1.0.0 → 客户端 `no-newer` 不强制下发，无用户面变更）。
- **同时修正一处无据注释（重要）**：`feed-backfill.ts` 头部原写「雪球实测滚动 2-3 屏后切换为按钮式」——该说法源自 09-09 §5.6 附注的推测，**本次真机取证直接推翻**（按钮全程不可见）。已改写为「滚动无限加载 + 该选择器按防御性配置声明」。

### 2. 切片 2 真机回归结论

| 场景 | 结果 |
|---|---|
| 白名单挂载 | 7x24、关注/全部 出现「查找更早的帖子」；自选/视频/基金/资讯/达人/私募/ETF 无操作条（`bar=null`）✅ |
| 滚动补拉（7x24，截止=昨日0点） | `y 0→1129`、行数 10→30、屏数累加 3 屏 ✅ |
| 命中路径（关注/全部，截止=今天09:15） | 1 屏新增 15 行、**命中 16 条**、「已找到 16 条更早的符合时间条件帖子（已加载 1 屏）」+ 按钮变「继续查找」✅ |
| 末页文案 | 浮动条「已到信息流末页，共找到 0 条符合时间条件（已加载 2/3 屏）」+ 按钮禁用；Popup「已加载 3 屏，到达信息流末页。」（`scan-text.ts` 双端一致）✅ |
| 计数联动 | Popup `已过滤: 30 条` 与探针 `filtN=30` 一致；切类目后按新上下文重置（关注/全部 = 11）✅ |
| 取消（cancelled） | **本次未重测**：关注流 1 屏即达目标 10 条回到 idle，1.2~2.5s 窗口短于工具点击延迟；沿用 09-09 double-click 真机记录 + 单测 ✅/◐ |
| 屏数上限（limit=10 屏） | **本次未走到**：站点 2-3 屏内即停滞，无法自然累积 10 屏；保留单测覆盖 ◐ |

### 3. 新发现缺陷（本批已修，单测覆盖）

**窗口已处于文档底部时重复触发补拉会误判「已到信息流末页」**：`window.scrollTo({top: scrollHeight})` 在已到底时为空操作 → 站点不再懒加载新行 → 连续 `end_stall_count=2` 屏即进 exhausted（实测第 2 屏即判「末页」，而信息流实际仍有内容）；且 exhausted 后按钮禁用，用户需切换类目/改设置才能重挂载重试。

**修法（批次十一内落地）**：新增 `scrollToFeedEnd()`——当文档确实可滚动（`scrollHeight - innerHeight > 0`）且 `scrollY` 已落在底部（容差 2px）时，先 `scrollTo(maxTop - innerHeight)` 回退一屏、`await sleep(min(250ms, scroll_delay_ms))` 留出可观察间隔（两次同步滚动会被合并成一帧、站点看不到「离开底部」），再滚到文档末端；未到底部时行为与原先完全一致。顺带把 `screens++` 移到滚动之后并补一次 `runId` 校验，避免取消瞬间多记一屏。

- 覆盖：`feed-backfill.test.ts` 新增第 8 节——8a「已处于底部 → 先回退一屏（`maxTop - innerHeight`）再滚到底」、8b「未到底 → 直接滚到末端且不产生多余回退」；JSDOM 下 `scrollHeight=0` 不满足「可滚动」前置条件，故既有用例的滚动次数与语义不变。
- **真机复验待下一设备会话**（本批无设备操作）：预期现象为「已在底部再次触发不再 2 屏即报末页，而是重新拉到新内容或继续累计屏数」。

### 4. O1 正式关闭（区间模式复核）

区间=**今天 09:30–11:30**（Popup「午间复盘」预设）+ `视频` 类别：`art.tli=8 / article=8`、**`filtN=8`（8 条视频卡片全部命中并被隐藏）**、`bar=null`。**区间模式下同样不存在「视频卡片绕过过滤链」**；结合 09-14 截止模式证据，O1 在两种模式下均不可复现 → 结案，无适配包改动。P2-16 第一项（首页区间版视频类）随之闭环。

### 5. P2-18 预设真机验收（交易日正路径）

交易日（周三）区间面板点击「午间复盘」→ 起止框正确填入 `2026/09/16 09:30 ~ 11:30`（当地日期时间控件逐字段核对），保存后内容脚本即时重滤。**交易日正路径首次真机走通**（此前仅周末回退分支）；法定节假日缺口仍按 v1.0 口径标注。

### 6. 取证方法沉淀（更新 09-14 记录）

1. **反调试规避**：雪球挂 DevTools 会触及其 `debugger` 陷阱 → 继续使用「内容脚本临时浮层探针」，本批升级为**每行独立 DOM 元素 + 14px 字号**：既能截图读，也能直接从无障碍树逐行读取全量文本（消除小字号误读——本批据此纠正了两次「0 行/0 条」的读数错误）。
2. **坐标点击不可靠的教训**：该页**类目标签区域的合成点击不生效**（无障碍 `elementFromPoint` 显示标签中心可命中，但 cg_event 点击被吞）；且本环境**栅格 1280 宽 vs 页面视口 1920 CSS 宽（dpr=1）**存在换算，凭肉眼估坐标多次点偏。**解法：探针内置地址栏 hash 通道**——`#tm-tab-<类目名>` → 探针用 `a.click()` 代点标签（纯键盘驱动、零坐标依赖）。**踩坑**：雪球路由会把 hash 规范化成 `#/`，必须从 `hashchange` 事件的 `newURL` 取值（读 `location.hash` 会被抢跑）。
3. 扩展页面的控件（Popup 预设等）用**无障碍 AXPress** 可靠，坐标点击同样不可靠。
4. 扩展重载后**其扩展页面（Popup 标签页）会被 Chrome 关闭**，需重新打开。

### 7. 验证与遗留

- 基线：`npm test` **17 个文件**全绿（sites-panel 的版本/last_verified 断言改为从适配包动态推导，避免每次合法取证都误报）、`npx tsc --noEmit` 0 错误、`rm -rf dist && npm run build`（loader ✅）、`npm run test:build`、`git diff --check`。
- 遗留（下一真机批次）：①**dist-store 形态**验证 P2-20 授权弹窗/「授权全部金融站点」/撤销 4 项 + §13 商店形态 E2E + §11 远程适配包真机观察（需切换构建并走授权链路）；②**本批缺陷修法 `scrollToFeedEnd()` 的真机复验**；③§5.1 其余类目区间版按需补测（本批仅视频类）；④详情页评论回归；⑤P2-17 取消态与 10 屏上限真机复测；⑥§10.1–10.3 其他平台真机验收（需各站登录态）。
- 仓库状态：`src/content/dom-probe.ts` 已删除、`index.ts` 还原，探针未进入任何提交。

---

## 2026-09-14 批次十（O1 视频卡片复核：结论「不可复现」+ P2-17 切片 2）

> 用户指令：清理 `.kilo`（已完成，见下）→ 做 O1 视频卡片修复 → 做 P2-17 切片 2。

### O1 复核结论（重要：**不改适配包**）

**结论：O1（「雪球视频卡片绕过过滤链」）在当前 DOM 下不可复现，不修改适配包。**

真机取证（登录态日常 Profile，2026-09-14，`视频` 类别，截止=2026-09-06 00:00）：
- `videos: 8` 且 `matched: 8`：**8 张视频卡片全部命中现有选择器 `article.timeline__item`**（`articleItem: 8`，视频位于卡片内 `div.video_box.timeline-video`）；
- 窗口外卡片确实被隐藏：`昨天 23:12 / 昨天 23:11 / 昨天 21:32` 三张视频卡片 `display: none`（`hidden: true, video: true`）；窗口内卡片正常显示；
- 即 09-09 批次五记录的「视频卡片完全绕过过滤链（无隐藏、无 unparseable 计数）」在今日 DOM 上不成立。**不做无证据的选择器增补**（若按当时结论加 `.video__box` 等，今日实测该 class 根本不存在——`videoBox: 0`，正是「猜测选择器」的风险示例）。

处置：O1 降级为「历史 DOM 变体 / 不可复现」，保留在 P2-16 第一项的注记中；**区间模式**（09-09 报告异常的原始场景）视频类复核排入下一真机批次；若复现，届时按同样的浮层探针法取证 DOM 后再改适配包。

**取证方法（沉淀，避免下次踩坑）**：雪球页面挂 DevTools 会周期性命中其反调试 `debugger` 陷阱（表现为反复「Paused in debugger」，需停用断点才能继续），导致 DevTools 路线不可用；改用**内容脚本临时浮层探针**（隔离世界读 DOM，不执行页面代码，把结果写进 fixed 浮层再截图读取）——仅在确认需要 DOM 取证时临时加入、取证后立即移除（本批已移除，未进入提交）。

### P2-17 切片 2 落地

1. **「加载更多」按钮式获取（数据驱动）**：
   - `FeedBackfillConfig.load_more_selector?`（types）+ schema 校验（非空字符串）；
   - 引擎：滚动无新增时，若适配包声明该选择器且按钮可用（存在/可见/未禁用）→ 点击后等待一个 `scroll_delay_ms` 再统计新增；按钮拉取同样计入 `max_screens`（一次用户可见拉取 = 1 屏）；按钮缺失/禁用时维持「无新增计停滞」原语义；文件头注释同步说明获取策略；
   - 测试 +4：按钮式拉取命中计入、按钮缺失维持末页语义、禁用不点击、按钮拉取计入屏上限；schema 空值拒绝 + 合法值通过。
   - **适配包暂不声明该选择器**：雪球「加载更多」按钮的 class 未取证（批次五仅观察到按钮存在），按 O1 教训不猜测选择器——取证后一行配置即可启用（见下批待办）。
2. **扫描状态纳入诊断报告**：新增 `src/shared/scan-text.ts`——`scanStatusText`（Popup 展示，从 popup/main.ts 迁出，行为不变）与 `scanDiagnosticLine`（脱敏诊断行 `扫描=loading | 单位=屏 | 已扫描 3/10 | 当前原始页=…`）唯一文案来源；Popup 复制报告新增该行（仅状态与计数，无页面内容/URL query/正文）；测试 +18（双单位 × 六状态、诊断行、脱敏断言）。
3. **原生页码策略收编（边界与计划，如实记录）**：本批完成**状态契约与文案的收编**（`ScanProgress` + `scan-text.ts` 成为虚拟分页与补拉共同的唯一契约/文案来源）；**引擎级收编**（把原生页码/下一页/加载更多/无限滚动收敛为统一 Strategy 对象驱动）**未做且不建议在 v1.0 做**：需要改写虚拟分页的会话/缓存/隐藏抵抗机制（项目风险最高、真机验证成本最高的代码），对 v1.0 无功能增益。已具备的替代事实：两条引擎均为数据驱动（适配包声明 `virtual_pagination` / `feed_context.backfill`），新增平台只需配置；切片 2 的按钮式获取已作为补拉引擎的第二种获取方式落地。建议在 P2-21 多平台适配中按需提取公共 Strategy 接口（届时有两个以上真实平台样本，抽象才有依据）。

### 仓库卫生（本批附带）

- `.kilo/`（本地工具目录）已从工作树与**全部历史**移除（`git filter-branch --index-filter` 清除 + 旧 refs/reflog/gc 清理 + force-push），并加入 `.gitignore`；远端与本地同为 `19e1088`。

### 验证与遗留

- 基线：`npm test` **17 个文件**全绿（新增 scan-status.test.ts，已挂 package.json；feed-backfill 增 4 组用例）、`npx tsc --noEmit` 0 错误、`rm -rf dist && npm run build`（loader ✅）、`npm run test:build`、`git diff --check`。
- 遗留：①雪球「加载更多」按钮选择器取证（取证后写入 `xueqiu.json`）；②区间模式视频类复核（O1 场景）；③P2-16 第一项仍不勾（区间版视频类待复核）；④切片 2 的真机回归（按钮式补拉实际行为）排入下一真机批次。
...
2. **P2-17 切片 2 开发**：load-more 按钮式获取（批次五实测依据：雪球滚动追加 2-3 屏后切「加载更多」按钮）+ 原生页码策略收编统一引擎 + 扫描状态纳入诊断报告。
3. **P2-19 发布源**（动工前置条件：**与用户确认外部资源**——GitHub 仓库/org 名、jsDelivr 路径、是否自建或依赖第三方；REMOTE_ADAPTERS_URL 现为 `cdn.jsdelivr.net/gh/org/repo@latest/adapters.json` 占位）+ 完整性校验/版本回退/应急停用。
4. 收尾：§14 材料更新（截图 2/3/6 登录态重截、隐私政策托管 URL 用户步骤）+ 发布准备核对（P2-23）。
...
- [x] 诊断明细补充页面类型、当前类别、采用的回退行为与复制报告（2026-08-18 起 Popup 明细含 {kind, page, context, raw, 回退行为} 与复制报告；页面类型以 pathname 表达；**2026-09-14 切片 2 补齐扫描状态行**——shared/scan-text.ts 统一文案并纳入脱敏报告）
...
- [ ] 首页全部目标类别、个股六类别、详情评论均通过截止与区间模式真机验收
  ...
  - 进度更新 2026-09-16：**区间版视频类复核通过（O1 正式关闭）**——区间=今天 09:30–11:30 下 8/8 视频卡片全部命中 `article.timeline__item` 并被隐藏（`filtN=8`），结合 09-14 截止模式取证，O1 两种模式均不可复现、无适配包改动；本项闭环（首页区间版视频类不再作为例外）。
...
- [ ] 统一获取策略配置：原生页码、下一页、加载更多、无限滚动、严格时间排序切换
  - ◐ 切片 1（2026-09-08）：无限滚动补拉已落地（feed-backfill.ts，数据驱动 feed_context.backfill）
  - ◐ 切片 2（2026-09-14）：**加载更多按钮式获取**已落地（`load_more_selector` + 引擎点击路径 + 4 组测试；雪球选择器待真机取证后配置）；原生页码策略收编明确不做（理由与计划见批次十记录：需改写最高风险的虚拟分页会话机制，v1.0 无功能增益，留待 P2-21 有第二个平台样本时提取公共 Strategy）
- [ ] 统一稳定 ID 去重、缓存、取消、加载超时、末页、重复页和扫描上限
  - ◐ 补拉侧已落地（切片 1 取消/末页/屏数上限/命中目标；按钮拉取计入上限——切片 2）；虚拟分页侧收编不做（同上）
...
- [ ] 统一可信状态：仅过滤已加载内容、正在跨页查找、已扫描 N 个原生页面、已到末页、非严格时间排序
  - ◐ ScanProgress 增 unit: 'pages'|'screens'（切片 1）；**文案与状态契约收编完成**（切片 2，shared/scan-text.ts 为唯一来源）；「非严格时间排序」以 contexts 白名单 + loaded-only 标注体现
- [x] 所有状态可被 Popup、悬浮条和设置页消费；“未找到”不得被表达成“网站没有”（2026-09-08 批次四：Popup scanStatusText 与悬浮条按 unit 渲染「屏/原生页」双语义；「未找到」始终表达为已扫描范围而非站点无内容；2026-09-14 切片 2：文案来源统一到 shared/scan-text.ts 并纳入脱敏诊断报告）
- [x] 用雪球个股页和至少一个首页可回溯类别做兼容回归，再允许新平台复用（切片 1 回归已完成；**切片 2 真机回归 2026-09-16 完成**：取证雪球「加载更多」= `a.home__timeline__more`（补拉全程 `display:none`，站点实为滚动无限加载，适配包按防御性声明 v0.4.1 配置）；7x24 与关注/全部 两上下文滚动补拉、命中累加（16 条/1 屏）、末页文案与按钮禁用、Popup 计数联动均真机通过；**取消态与 10 屏上限本次未重测**（窗口过短/站点早停滞），沿用 09-09 记录与单测；**新登记缺陷**：已在文档底部时重复触发会误判末页，见 `feed-backfill.ts` 头部「已知限制」）
...
- [ ] 诊断报告包含平台、页面 URL/类型、类别、原始时间文本、回退行为、适配包版本和扫描状态
  - ◐ 部分完成（2026-08-18）：Popup 复制脱敏报告已含平台/适配包版本/模式边界/计数与逐条 {kind, page, context, raw, 回退行为}
  - ◐ 2026-09-14 切片 2：**扫描状态已纳入**（`扫描=state | 单位=屏/原生页 | 已扫描 N/M | 当前原始页`，文案来源 shared/scan-text.ts，含脱敏断言）；仍缺「页面类型」字段（当前以 pathname 近似表达）
